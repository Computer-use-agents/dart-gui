...
if __name__ == '__main__':
    ...
    raw_data_dir = "results/pass@32_trainset90"
    splitter = TrajectorySplitter(
                root_dir=raw_data_dir,
                window_size=5,
                stride_size=5
    ...
    with open('data_pass@8_train8.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
    trajectory_ids = [item['trajectory_id'] for item in data if 'trajectory_id' in item]
    ...
    for i, trajectory_id in enumerate(trajectory_ids):
        with open(os.path.join(raw_data_dir, trajectory_id, 'final_messages.json'), 'r', encoding='utf-8') as f:
            full_messages = json.load(f)
        
        num_chunks = splitter.split_and_save(
        ...
        if i % 8 == 7:
            print(task_num)
            task_num = 0  
    print(f"Number of chunks created: {num}")