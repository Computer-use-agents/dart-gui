...
class TrajectoryRunnerActor:
        ...
        try:
            ...
            while step < self.max_steps and not done:
                ...
                if step + 1 == self.max_steps:
                    ...
                    st = time.time()
                    if model_pool:
                        response, model_path, vllm_logp = await self._call_model(
                            model_pool, messages,
                ...
                # ---- save vllm logp ----
                await storage.save_partial_vllm_logp.remote(self.task_root, step + 1, vllm_logp)
    ...
    async def _call_model(self, model_pool, messages: str, model_cfg):
        ...
        logprobs = model_cfg.get('logprobs', False)
        
        for attempt in range(1, retries + 1):
            try:
                response, model_path, vllm_logp = await asyncio.wait_for(
                ...
                return response, model_path, vllm_logp
