...
class MySQLRolloutORM:
    ...
    def insert_checkpoint(self, path: str):
        ...
        with self.session_scope() as s:
            # 获取当前最大 i
            result = s.execute(
                text("SELECT COALESCE(MAX(CAST(SUBSTRING(name, 12) AS UNSIGNED)), 0) "
                    "FROM checkpoint")
            )
            max_i = result.scalar() or 0
            next_i = max_i + 1

            name = f"checkpoint_{next_i}"
            version = f"v{next_i}"

            s.execute(
                text("INSERT INTO checkpoint (name, path, version, status) "
                    "VALUES (:n, :p, :v, :st)"),
                {"n": name, "p": abs_path, "v": version, "st": 1}
            )
            logger.info(
                f"Inserted checkpoint: name={name}, version={version}, path={abs_path}, status=1"
            )
